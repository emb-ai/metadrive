...
def _lanes_ahead_from_checkpoints(nav, lane_idx):
    ...
    if ckpt_idx is None or map_net is None or not hasattr(map_net, "road_network"):
        return []
    rn = map_net.road_network
    try:
        start_i = max(0, int(ckpt_idx[0]) if hasattr(ckpt_idx, "__getitem__") else 0)
    except Exception:
        start_i = 0

    # EdgeRoadNetwork (SUMO, OSM nets): graph[lane_id] = lane_info. NodeRoadNetwork uses graph[a][b] = [lanes].
    if isinstance(rn, EdgeRoadNetwork):
        out = []
        graph = rn.graph
        for i in range(start_i, len(ckpts)):
            ck = ckpts[i]
            if ck not in graph:
                continue
            peers = rn.get_peer_lanes_from_index(ck)
            if not peers:
                continue
            li = min(int(lane_idx), len(peers) - 1)
            lane = peers[li]
            if out and out[-1] is lane:
                continue
            out.append(lane)
        return out
